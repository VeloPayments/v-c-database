/**
 * \file test_database_open_from_builder.cpp
 *
 * \brief Test the vcdb_database_open_from_builder() method.
 *
...
/**
 * Test that the open_open_builder method calls the appropriate engine
 * methods.
 */
TEST(database_open_from_builder, init)
{
    ...
    /* preconditions */
    ASSERT_FALSE(test_database_open_called);

    /* the opened flag is NOT set in the builder. */
    ASSERT_FALSE(builder.database_opened);

    /* we should be able to open our database */
    ASSERT_EQ(VCDB_STATUS_SUCCESS,
        vcdb_database_open_from_builder(
            &database,
            &builder));

    /* the test_database_open method was called. */
...
/**
 * Test that passing bad parameters to open_from_builder results in an error
 * return code.
 */
TEST(database_open_from_builder, bad_param)
{
    vcdb_builder_t builder;
    vcdb_database_t database;
 ...
 * open_from_builder and if this is an error code, the database is not marked
 * as open.
 */
TEST(database_open_from_builder, engine_error)
{
    vcdb_builder_t builder;
    vcdb_database_t database;
    ...
    /* preconditions */
    ASSERT_FALSE(test_database_open_called);
    test_database_open_retval = TEST_DATABASE_OPEN_ERROR_CODE;
